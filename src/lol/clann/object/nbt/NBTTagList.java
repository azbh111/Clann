...
public class NBTTagList extends NBTBase implements List<NBTBase> {
    ...
    public String toJson(boolean withColor) {
        ...
        if (size() > 0) {
            ...
            for (int i = 0; i < size() - 1; i++) {
                sb.append(index_prefix).append(i).append(index_subfix).append(":").append(get(i).toJson(withColor)).append(",");
            }
            sb.append(index_prefix).append(size() - 1).append(index_subfix).append(":").append(get(size() - 1).toJson(withColor));
        }
        if (withColor) {
            sb.append("§5]");
        } else {
    ...
    public Number getNumber(int i) {
        NBTBase t = get(i);
        if (t instanceof NBTTagNumeric) {
            return (Number) ((NBTTagNumeric) t).get();
        }
        return null;
    }
    ...
    public NBTBase[] toArray() {
        ...
        int i = 0;
        for (Object tag : list) {
            bases[i++] = NBTBase.wrap(NBTBase.cloneHandle(tag));
        }
        return bases;
    }
