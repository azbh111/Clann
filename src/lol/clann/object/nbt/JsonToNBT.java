...
import lol.clann.object.nbt.exception.NBTException;

public class JsonToNBT {


    public static NBTBase wrap(String p_150315_0_) {
    ...
    static JsonToNBT.Any func_150316_a(String p_150316_0_, String p_150316_1_) {
        ...
        } else if (p_150316_1_.startsWith("[") && !p_150316_1_.matches("\\[[-\\d|,\\s]+(\\]|\\]b)")) {
            if (!p_150316_1_.endsWith("]")) {
                throw new NBTException("Unable to locate ending bracket for: " + p_150316_1_);
            } else {
                p_150316_1_ = p_150316_1_.substring(1, p_150316_1_.length() - 1);
    ...
    public static class Compound extends JsonToNBT.Any {
        ...
        public NBTBase wrap() {
            ...
            while (iterator.hasNext()) {
                ...
                nbttagcompound.put(any.field_150490_a, any.wrap());
            }
    ...
    public static class Primitive extends JsonToNBT.Any {
        ...
        public static NBTTagDatable wrap(String json) {
            return (NBTTagDatable) new Primitive(null, json).wrap();
        }
            ...
            try {
                if (this.field_150493_b.matches("[-+]?[0-9]*\\.?[0-9]+[d|D]")) {
                    ...
                    if (this.field_150493_b.startsWith("[")) {
                        if (this.field_150493_b.endsWith("]")) {
                        ...
                        } else if (this.field_150493_b.endsWith("]b")) {
                            if (this.field_150493_b.length() > 3) {
                                String s = this.field_150493_b.substring(1, this.field_150493_b.length() - 2);
                                String[] astring = s.split(",");
                                try {
                                    if (astring.length <= 1) {
                                        return new NBTTagByteArray(new byte[]{Byte.parseByte(s.trim())});
                                    } else {
                                        byte[] aint = new byte[astring.length];
                                        for (int i = 0; i < astring.length; ++i) {
                                            aint[i] = Byte.parseByte(astring[i].trim());
                                        }
                                        return new NBTTagByteArray(aint);
                                    }
                                } catch (NumberFormatException numberformatexception) {
                                    return new NBTTagString(this.field_150493_b);
                                }
