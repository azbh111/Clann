...
import lol.clann.object.nbt.NBTTagList;
import lol.clann.object.nbt.exception.NBTReadException;
...
final class NBTUtils_MCPC_named extends NBTUtils {
    ...
    private RefConstructor con_NBTagByte = class_NBTTagByte.getConstructor(new Object[]{String.class, byte.class});
    private RefConstructor con_NBTagShort = class_NBTTagShort.getConstructor(new Object[]{String.class, short.class});
    private RefConstructor con_NBTagInt = class_NBTTagInt.getConstructor(new Object[]{String.class, int.class});
    private RefConstructor con_NBTagLong = class_NBTTagLong.getConstructor(new Object[]{String.class, long.class});
    private RefConstructor con_NBTagFloat = class_NBTTagFloat.getConstructor(new Object[]{String.class, float.class});
    private RefConstructor con_NBTagDouble = class_NBTTagDouble.getConstructor(new Object[]{String.class, double.class});
    private RefConstructor con_NBTagString = class_NBTTagString.getConstructor(new Object[]{String.class, String.class});
    private RefConstructor con_NBTagByteArray = class_NBTTagByteArray.getConstructor(new Object[]{String.class, byte[].class});
    private RefConstructor con_NBTagIntArray = class_NBTTagIntArray.getConstructor(new Object[]{String.class, int[].class});
    private RefConstructor con_NBTagCompound = class_NBTTagCompound.getConstructor(new Object[]{String.class});
    private RefConstructor con_NBTagList = class_NBTTagList.getConstructor(new Object[]{String.class});
    ...
    NBTUtils_MCPC_named() {
        if (met_NBTBase_load_args == 0) {
            try {
                ...
                con_NBTReadLimiter = class_NBTReadLimiter.getConstructor(new Object[]{long.class});
    ...
    public Object getValue(Object tag) throws NBTReadException {
        ...
        if (!class_NBTBase.isInstance(tag)) {
            throw new NBTReadException(tag);
        }
        switch (getTagType(tag)) {
            ...
            case 9:
                return field_NBTagList_list.of(tag).get();
            case 10:
                return field_NBTagCompound_map.of(tag).get();
            case 11:
    ...
    protected void setRawValue(Object tag, Object value) throws NBTReadException {
        ...
        switch (getTagType(tag)) {
            ...
            case 8:
                ...
                break;
            case 9:
                NBTTagList list = new NBTTagList(true, tag);
                list.clear();
                ...
                break;
            case 10:
                NBTTagCompound compound = new NBTTagCompound(true, tag);
                compound.clear();
                for(Map.Entry<String, Object> m:((Map<String,Object>)value).entrySet()){
                    compound.put(m.getKey(), m.getValue());
                }
                break;
            case 11:
