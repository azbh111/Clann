...
import lol.clann.object.nbt.exception.NBTConvertException;
import lol.clann.object.nbt.exception.NBTReadException;
import static lol.clann.object.nbt.utils.ChunkUtils.chunkUtils;
import static lol.clann.object.nbt.utils.EntityUtils.entityUtils;
import static lol.clann.object.nbt.utils.ItemStackUtils.itemStackUtils;
...
public abstract class NBTUtils {
    ...
    static {
        ...
        if (isForge()) { // Forge classpath
            try { // to get raw constructor
                getRefClass("{nm}.nbt.NBTTagByte, {NBTTagByte}").getConstructor(new Object[]{byte.class});
                raw = true; // on success
            ...
            try { // to get raw constructor
                getRefClass("{nms}.NBTTagByte, {NBTTagByte}").getConstructor(new Object[]{byte.class});
                raw = true; // on success
            } catch (Exception ignored) {
            }
    ...
    public Object createTag(Object javaObject) throws NBTConvertException {
        ...
        if (javaObject instanceof Boolean) {
            if ((Boolean) javaObject) {
                return createTagByte((byte) 1);
            } else {
                return createTagByte((byte) 0);
            }
        }
        if (javaObject instanceof Collection) {
            return new NBTTagList(true,(Collection) javaObject).getHandle();
        }
        if (javaObject instanceof Short) {
            return createTagShort((Short) javaObject);
        }
        if (javaObject instanceof CharSequence) {
            return createTagString(javaObject.toString());
        }
        if (javaObject instanceof Byte) {
            return createTagByte((Byte) javaObject);
        }
        if (javaObject instanceof Integer) {
            return createTagInt((Integer) javaObject);
        }
        if (javaObject instanceof Float) {
            return createTagFloat((Float) javaObject);
        }
        if (javaObject instanceof Double) {
            return createTagDouble((Double) javaObject);
        }
        if (javaObject instanceof Long) {
            return createTagLong((Long) javaObject);
        }
        if (javaObject instanceof BigDecimal) {
            return createTagDouble(((BigDecimal) javaObject).doubleValue());
        }
        if (javaObject instanceof BigInteger) {
            return createTagLong(((BigInteger) javaObject).longValue());
        }
        if (javaObject instanceof UUID) {
            return createTagString(javaObject.toString());
        }
        if (javaObject instanceof byte[]) {
            return createTagByteArray((byte[]) javaObject);
        }
        if (javaObject instanceof int[]) {
            return createTagIntArray((int[]) javaObject);
        }
        ...
        switch (type) {
            ...
            case 1: {
                ...
                if (value instanceof CharSequence) {
                    return new Long((String) value).byteValue();
                }
                throw new NBTConvertException(value, type);
            }
            case 2: {
                ...
                if (value instanceof CharSequence) {
                    return new Long(value.toString()).shortValue();
                }
                throw new NBTConvertException(value, type);
            }
            case 3: {
